...
Face detection and face recognition are two actively-researched and important topics in computer vision. Both of these tasks are performed frequently and easily by humans however no current system exists that can be deployed effectively in an unconstrained setting \cite{sinha2006face}.

Earlier forms of facial detection and recognition involved holistic methods such as Eigenfaces\cite{turk1991eigenfaces} and Fisherfaces\cite{belhumeur1997eigenfaces}. However, more recently, local descriptors such as Local Binary Patterns (LBP) and Local Intensity Distribution (LID) have gained attention due to their efficiency and tolerance to pose and illumination changes\cite{ahonen2004face}\cite{nguyen2011local}.
...
FIXME: History of FACE DETECTION
